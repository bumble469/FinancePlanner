...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!type || !Object.values(WorkItemType).includes(type)) {
      return NextResponse.json({ success: false, error: 'Invalid type. Must be PROJECT, EVENT or PLAN' }, { status: 400 });
    }
    ...
    const plan = await prisma.$transaction(async (tx) => {
      ...
      if (type === WorkItemType.PROJECT) {
        await tx.project.create({ data: { workItemId: workItem.id } });
      } else if (type === WorkItemType.EVENT) {
        await tx.event.create({ data: { workItemId: workItem.id } });
      } else if (type === WorkItemType.PLAN) {
        await tx.planInfo.create({ data: { workItemId: workItem.id } });
      }
